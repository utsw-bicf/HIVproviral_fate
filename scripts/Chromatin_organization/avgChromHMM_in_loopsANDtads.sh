...
grep -v "_" /project/BICF/BICF_Core/shared/Projects/Dorso/Chromatin_organization/HOMER/std_HicHomerTagDir/merge_lib/lib.tad.2D.bed | awk '{OFS="\t"}; $1 == $4 {print $1, ($3+$2)/2, ($5+$6)/2, $9}' | sort -k 1,1 -k 2,2n >/project/BICF/BICF_Core/shared/Projects/Dorso/Chromatin_organization/HOMER/std_HicHomerTagDir/merge_lib/lib.tad.2D_filtered.bed
...
awk '$1 == "chr1" && $2 > 1945500 && $3 < 2833500 {t = $4*($3-$2)} {sum+=t}END {print sum/(2833500-1945500)}' chromHMM_15_histone.bed
