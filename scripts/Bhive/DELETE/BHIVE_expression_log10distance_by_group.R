### This R script makes a jitter scatter plot of HIV expression data, log10(distance to nearest TSS) for each of the 5 groups
...
### Calculate the log10 of the distance to nearest TSS
df$log10dis <- log10(df$V23)

### Make plot, jitter scatter plot
pdf("BHIVE_expression_log10distance_by_group.pdf")
ggplot(df, aes(x=group, y=log10dis)) + 
  geom_jitter(position=position_jitter(width=.2, height=0)) +
  ggtitle("HIV Distance to TSS by Cluster") + 
  ylab("Distance in bp (log10)") +
...
#############################################################################
### Try without the log10 of distance

### Make plot, jitter scatter plot
pdf("BHIVE_expression_distance_by_group.pdf")
ggplot(df, aes(x=group, y=V23)) + 
  geom_jitter(position=position_jitter(width=.2, height=0)) +
  ggtitle("HIV Distance to TSS by Cluster") + 
  stat_summary(fun.y= mean, fun.ymin=mean, fun.ymax=mean, geom="crossbar", width=0.5, color="red") +
  scale_x_discrete(labels = wrap_format(10)) +
  scale_y_continuous(name = "Distance to TSS (in 1M bp)", breaks=c(0,500000, 1000000, 1500000), labels = c(0, 0.5, 1, 1.5)) +
  theme(plot.title = element_text(size=22, hjust = 0.5), 
        axis.title.x=element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
