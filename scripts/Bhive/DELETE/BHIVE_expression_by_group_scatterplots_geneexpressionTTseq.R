...
### Load TTseq data
logCPM <- read.table("/project/BICF/BICF_Core/shared/Projects/Dorso/Expression/rnaseq/workflow/output_TT/countTable.logCPM.txt", header=T, sep="\t")
logCPM$mean <- rowMeans(logCPM[c('Ttseq_CRAMER_GSM2260187', 'Ttseq_CRAMER_GSM2260188')], na.rm=TRUE)

fpkm <- read.table("/project/BICF/BICF_Core/shared/Projects/Dorso/Expression/rnaseq/workflow/output_TT/countTable.fpkm.txt", header=T, sep="\t")
fpkm$log10fpkm <- log(rowMeans(fpkm[c('Ttseq_CRAMER_GSM2260187', 'Ttseq_CRAMER_GSM2260188')], na.rm=TRUE)+1)
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_all.pdf")
ggplot(dffpkm, aes(x=V11, y=log10fpkm, size=V8, color=group)) + 
  ...
  ylab("Host Gene Expression - TTseq (Log10(fpkm))") + 
  scale_x_continuous(limits=c(-3.5, 3)) +
  scale_y_continuous(limits=c(0, 6.5)) +
  annotate("text", x = -3.5, y = 6.40, label = "Intergenic Same r = 0.104 (p-value = 0.244)", size=3, color = "#0072B2", hjust=0) + ###This line needs to be manually changed
  annotate("text", x = -3.5, y = 6.25, label = "Intergenic Convergent r = 0.184 (p-value = 0.124)", size=3, color = "#CC79A7", hjust=0) + ###This line needs to be manually changed
  annotate("text", x = -3.5, y = 6.10, label = "Intergenic Divergent r = 0.087 (p-value = 0.452)", size=3, color = "#D55E00", hjust=0) + ###This line needs to be manually changed
  annotate("text", x = -3.5, y = 5.95, label = "Intragenic Same r = -0.019 (p-value = 0.647)", size=3, color = "#009E73", hjust=0) + ###This line needs to be manually changed
  annotate("text", x = -3.5, y = 5.8, label = "Intragenic Convergent r = 0.086 (p-value = 0.03)", size=3, color = "#F0E442", hjust=0) + ###This line needs to be manually changed
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_group1.pdf")
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_group2.pdf")
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_group3.pdf")
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_group4.pdf")
...
pdf("BHIVE_expression_scatterplot_TTSEQexpression_group5.pdf")
